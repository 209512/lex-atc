...
export const SidebarControlPanel = () => {
    ...
    const { isDark, sidebarWidth, uiPreferences, updateUIPreferences, setSidebarWidth } = useUIStore(useShallow(s => ({ isDark: s.isDark, sidebarWidth: s.sidebarWidth, uiPreferences: s.uiPreferences, updateUIPreferences: s.updateUIPreferences, setSidebarWidth: s.setSidebarWidth })));
    
    const [isOverrideLoading, setIsOverrideLoading] = React.useState(false);
    const { viewMode } = uiPreferences;
    const showEmergencyLabel = sidebarWidth >= 340;
    ...
    return (
        <div className={clsx(
            "p-2.5 border-b z-20 relative shrink-0 flex items-center gap-2 h-20 min-w-0",
            ...
            <div className="flex flex-col gap-1 min-w-0">
                <Tooltip content="Switch View Mode" position="bottom-left">
                    ...
                    >
                        {viewMode === 'operator' && <Activity size={16} className="shrink-0" />}
                        {viewMode === 'executive' && <LayoutDashboard size={16} className="shrink-0" />}
                        {viewMode === 'focus' && <Eye size={16} className="shrink-0" />}
            ...
            <div className="flex items-center h-full min-w-0 flex-1 tour-step-emergency">
                {isHuman ? (
                    ...
                    >
                        ...
                        {showEmergencyLabel && <span className="whitespace-nowrap">Release Lock</span>}
                    </button>
                ) : (
                        ...
                        >
                            {isOverrideLoading ? (
                                ...
                                <>
                                    ...
                                    {showEmergencyLabel && <span className="whitespace-nowrap">Emergency Takeover</span>}
                                </>
                            )}
                        </button>
                    </Tooltip>
