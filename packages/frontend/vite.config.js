...
export default defineConfig(({ mode }) => {
  ...
  const enableProxy = !enableMsw && Boolean(proxyTarget);

  const apiGuardPlugin = {
    name: 'lex-atc-api-guard',
    configureServer(server) {
      ...
      server.middlewares.use((req, res, next) => {
        ...
        res.end(JSON.stringify({ error: 'MSW_REQUIRED' }));
      });
    }
  };

  return {
    plugins: [react(), apiGuardPlugin],
    resolve: {
      alias: {
